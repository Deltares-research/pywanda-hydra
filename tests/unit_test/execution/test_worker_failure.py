...
from pywandahydra.execution.locking import CaseLock
...
class TestWorkerFailurePath(unittest.TestCase):
    def test_run_one_case_reports_failure_and_persists_failed_status(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            state = CaseStatusStore(plan.case_dir).read()

            self.assertEqual(
                state.simulation_status,  # type: ignore[union-attr]
                "failed",
            )
            self.assertEqual(
                state.postprocessing_status,  # type: ignore[union-attr]
                "failed",
            )
            self.assertIn(
                "simulated steady-state failure",
                state.error_summary,  # type: ignore[arg-type, union-attr]
            )
    ...
    def test_run_one_case_returns_failure_when_case_dir_locked(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertFalse(result["success"])
            self.assertIn(".case.lock", result["error"])
